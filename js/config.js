// Game Configuration Constants
export const CONFIG = {
  CANVAS: {
    WIDTH: 1200,
    HEIGHT: 600,
  },

  CANNON: {
    X: 120,
    Y: 475,
    BASE_FIRE_RATE: 3000, // ms between shots
    BASE_SIZE: 8,
    SPEED: 20,
    SPEED_MIN: 20,
    SPEED_MAX: 20,
    WEIGHT: 1.0,
    MIN_ANGLE: -10, // -10 degrees
    MAX_ANGLE: Math.PI / 2, // 90 degrees
  },

  CASTLE: {
    X: 900,
    Y: 450,
    MIN_WIDTH: 8,
    MAX_WIDTH: 12,
    MIN_HEIGHT: 12,
    MAX_HEIGHT: 15,
    BASE_REWARD: 15,
  },

  UPGRADES: {
    BASE_COSTS: {
      fireRate: 10,
      size: 50,
    },
    COST_MULTIPLIER: 1.5,
    LEVEL_CAPS: {
      fireRate: 14,
      size: 10,
    },
    EFFECTS: {
      fireRate: 0.08, // 8% faster per level
      size: 0.1, // 10% larger per level
    },
  },

  PHYSICS: {
    GRAVITY: 1.0,
    WOOD_HEALTH: 4,
    STONE_HEALTH: 8,
    BLOCK_SIZE: 25,
    GROUND_Y: 500,
  },

  COLORS: {
    WOOD: "#8B4513",
    STONE: "#708090",
    CANNON: "#2F4F4F",
    GRASS: "#32CD32",
    SKY: "#87CEEB",
    CANNONBALL: "#2F2F2F",
  },

  WORLDS: [
    {
      id: 1,
      name: "Earth",
      gravity: 1.0,
      speed: 20,
      colors: {
        sky: "#87CEEB",
        grass: "#32CD32",
        wood: "#8B4513",
        stone: "#708090",
        cannonball: "#2F2F2F",
      },
      completionRequired: 5, // Castles needed after maxing upgrades
    },
    {
      id: 2,
      name: "Moon",
      gravity: 0.17,
      speed: 20,
      colors: {
        sky: "#1a1a1a",
        grass: "#C0C0C0",
        wood: "#A0A0A0",
        stone: "#808080",
        cannonball: "#D3D3D3", // Lighter gray for visibility against dark sky
      },
      completionRequired: 5,
    },
    {
      id: 3,
      name: "Mars",
      gravity: 0.38,
      speed: 20,
      colors: {
        sky: "#CD853F",
        grass: "#B22222",
        wood: "#A0522D",
        stone: "#8B4513",
        cannonball: "#2F2F2F",
      },
      completionRequired: 5,
    },
    {
      id: 4,
      name: "Mercury",
      gravity: 0.38,
      speed: 20,
      colors: {
        sky: "#2F2F2F",
        grass: "#696969",
        wood: "#555555",
        stone: "#444444",
        cannonball: "#E5E5E5", // Lighter gray for visibility against dark sky
      },
      completionRequired: 5,
    },
    {
      id: 5,
      name: "Venus",
      gravity: 0.9,
      speed: 20,
      colors: {
        sky: "#FFA500",
        grass: "#FFD700",
        wood: "#DAA520",
        stone: "#B8860B",
        cannonball: "#2F2F2F",
      },
      completionRequired: 5,
    },
    {
      id: 6,
      name: "Jupiter",
      gravity: 2.36,
      speed: 30,
      colors: {
        sky: "#DEB887",
        grass: "#8B4513",
        wood: "#654321",
        stone: "#5D4E37",
        cannonball: "#2F2F2F",
      },
      completionRequired: 5,
    },
    {
      id: 7,
      name: "Saturn",
      gravity: 1.06,
      speed: 23,
      colors: {
        sky: "#F4A460",
        grass: "#DAA520",
        wood: "#B8860B",
        stone: "#9ACD32",
        cannonball: "#2F2F2F",
      },
      completionRequired: 5,
    },
    {
      id: 8,
      name: "Uranus",
      gravity: 0.89,
      speed: 20,
      colors: {
        sky: "#4FD0E3",
        grass: "#40E0D0",
        wood: "#20B2AA",
        stone: "#008B8B",
        cannonball: "#2F2F2F",
      },
      completionRequired: 5,
    },
    {
      id: 9,
      name: "Neptune",
      gravity: 1.13,
      speed: 24,
      colors: {
        sky: "#4169E1",
        grass: "#1E90FF",
        wood: "#0000CD",
        stone: "#000080",
        cannonball: "#2F2F2F",
      },
      completionRequired: 5,
    },
    {
      id: 10,
      name: "Pluto",
      gravity: 0.06,
      speed: 20,
      colors: {
        sky: "#191970",
        grass: "#B0C4DE",
        wood: "#778899",
        stone: "#708090",
        cannonball: "#F0F8FF", // Light blue-white for visibility against dark midnight blue sky
      },
      completionRequired: 5,
    },
    {
      id: 11,
      name: "Ceres",
      gravity: 0.03,
      speed: 20,
      colors: {
        sky: "#2F4F4F",
        grass: "#708090",
        wood: "#696969",
        stone: "#556B2F",
        cannonball: "#39FF14",
      },
      completionRequired: 5,
    },
  ],

  MONEY: {
    BASE_MULTIPLIER: 1,
    MAX_STREAK_MULTIPLIER: 10,
  },

  PRESTIGE: {
    BASE_GEM_REWARD: 50, // Gems earned on first prestige
    GEM_PER_CASTLE: 1, // Gems earned per castle after first prestige
    UPGRADE_BASE_COST: 50, // Base cost for prestige upgrades
    UPGRADE_COST_MULTIPLIER: 2, // Cost multiplier for prestige upgrades
    INCOME_BONUS_PER_LEVEL: 0.1, // 10% income bonus per prestige level
    UPGRADE_LEVEL_CAPS: {
      doubleShot: 10,
      fasterReload: 10,
      blastShot: 10,
      fireballs: 10,
      betterCastles: 10,
      passiveIncome: 10,
    },
    UPGRADES: {
      doubleShot: {
        name: "Double Shot",
        description: "Chance to fire two cannonballs simultaneously",
      },
      fasterReload: {
        name: "Faster Reload",
        description:
          "Reduces time between castle destruction and new castle spawn",
      },
      blastShot: {
        name: "Blast Shot",
        description: "Chance to fire a super-fast horizontal cannonball",
      },
      fireballs: {
        name: "Fireballs",
        description:
          "Chance for explosive cannonballs that damage nearby blocks",
      },
      betterCastles: {
        name: "Better Castles",
        description:
          "Smaller castles at low streaks, bigger at high streaks for optimal progression",
      },
      passiveIncome: {
        name: "Passive Income",
        description: "Earn money automatically over time",
      },
    },
    CANNONS: [
      {
        id: 0,
        name: "Medieval Cannon",
        description: "The classic castle destroyer",
        unlocked: true,
      },
      {
        id: 1,
        name: "Pirate Cannon",
        description: "Weathered wood with iron cannonballs",
        prestigeLevel: 1,
      },
      {
        id: 2,
        name: "WW1 Artillery",
        description: "Military green with explosive shells",
        prestigeLevel: 2,
      },
      {
        id: 3,
        name: "WW2 Tank Cannon",
        description: "Armored steel with armor-piercing rounds",
        prestigeLevel: 3,
      },
      {
        id: 4,
        name: "Modern Tank",
        description: "Desert camo with high-velocity shells",
        prestigeLevel: 4,
      },
      {
        id: 5,
        name: "Mortar",
        description: "Compact design with arcing projectiles",
        prestigeLevel: 5,
      },
      {
        id: 6,
        name: "Bazooka",
        description: "Rocket launcher with missile projectiles",
        prestigeLevel: 6,
      },
      {
        id: 7,
        name: "Missile Launcher",
        description: "Futuristic guided missiles",
        prestigeLevel: 7,
      },
      {
        id: 8,
        name: "Futuristic Cannon",
        description: "Energy weapon with plasma projectiles",
        prestigeLevel: 8,
      },
    ],
  },
};

// Material types
export const MATERIALS = {
  WOOD: "wood",
  STONE: "stone",
};

// Game states
export const GAME_STATES = {
  PLAYING: "playing",
  CASTLE_DESTROYED: "castle_destroyed",
};
